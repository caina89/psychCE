...
if( file.exists( 'ExtendedDataFigure4.Rdata' ) ){
  load( 'ExtendedDataFigure4.Rdata' )
} else {
  niter <- 1e3
  N <- 1e4
  M <- 1e2
...
for( it in 1:niter ){
  if( any( !is.na( Dirtest_oracle[,,it] ) ) ) next
  
  # simulate two subtype liabilities
  G <- matrix(rnorm(N*M),N,M)
  B <- matrix(rnorm(M*2),M,2) * sqrt(h2/M)
  Y <- G %*% B + matrix(rnorm(N*2),N,2) * sqrt(1-h2)
  ...
  pdf( 'ExtendedDataFigure4.pdf', w=10, h=10 )
  par( mfrow=c(2,2) )
  ...
  save.image( file='ExtendedDataFigure4.Rdata' )
}